...
export default (sbp('sbp/selectors/register', {
  ...
  'chelonia/defineContract': function (contract) {
    ...
    for (const action in contract.actions) {
      ...
      this.defContractSelectors.push(...sbp('sbp/selectors/register', {
        [`${contract.manifest}/${action}/process`]: (message, state) => {
        ...
        [`${contract.manifest}/${action}/sideEffect`]: async (message, state) => {
  ...
  'chelonia/out/eventsBetween': async function (startHash, endHash, offset = 0) {
    if (offset < 0) {
    ...
    const events = await fetch(`${this.config.connectionURL}/eventsBetween/${startHash}/${endHash}?offset=${offset}`)
      .then(handleFetchResult('json')).catch(console.error)

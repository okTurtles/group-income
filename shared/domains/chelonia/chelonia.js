...
import './files.js'
import './time-sync.js'
import { buildShelterAuthorizationHeader, clearObject, checkCanBeGarbageCollected, eventsAfter, findForeignKeysByContractID, findKeyIdByName, findRevokedKeyIdsByName, findSuitableSecretKeyId, getContractIDfromKeyId } from './utils.js'
...
export default (sbp('sbp/selectors/register', {
  ...
  'chelonia/reset': async function (postCleanupFn) {
    ...
    // We should do the following (i.e., remove all keys), but it breaks Vuex
    // (submodules) so instead we remove everything that looks like a contract
    // // reactiveClearObject(rootState, this.config.reactiveDel)
    Object.keys(rootState).filter(k => k.startsWith('z9br')).forEach(k => this.config.reactiveDel(rootState, k))

...
- **Coverage gap found after Step 9, now planned as Step 9a.** Flow ran with `all=true`, so it checked every non-ignored file, annotated or not — but the conversion set was defined by "contains annotations". **25 `.js` files under `frontend/` were Flow-checked and are checked by nothing today** (`checkJs: false`). Not theoretical: `mainNotificationsMixin.js` has two `$FlowFixMe[incompatible-use]` comments and has never had a single annotation, so Flow was demonstrably reporting errors there. Measured cost to close it: **16 errors in 2 files** (`vError.js` 5, `main.js` 5 — `window.Cypress`/`window.sbp` ambients and a vuelidate `$v` gap), the other 23 files clean. `common.js` is blocked and stays `.js`: `Gruntfile.js:679` matches it as a literal string in the slim-contract `external`, and every pinned slim contract from 2.0.0 to 2.9.0 carries `__require("@common/common.js")`. Full plan in TYPESCRIPT-MIGRATION-PLAN.md.

...
**Step 7 granted none. Step 8 granted one.** `confettiComponents/index.ts` — `confettiNames` was `Array<"confetii-triangle" | "confetti-circle" | "confetti-logo" | "confetti-rectangle">` assigned from `Object.keys()`. Flow accepted that (verified with a control line in the same file: only the control errored); TypeScript types `Object.keys` as `string[]` and reports TS2322, so keeping the annotation takes a cast. **Dropped**, per "prefer dropping" — it did not give callers `any`, and the one consumer, `randomFromArray` in `AnimationMixins.ts`, has an unannotated parameter, so the union was checking nothing where it could have. The file carries the comment.
...
| **7 — DONE** | `frontend/controller/**`: root 3, `actions` 9, `app` 3, `e2e` 1, `serviceworkers` 3, `utils` 1, **+3 pulled forward from Step 8** | **23** | Controller root holds 4 Flow files; `service-worker.js` is Flow-ignored (strip-only), leaving 3. `serviceworkers/sw-primary.js` is its own esbuild entry — update `Gruntfile.js:663` in the same commit (Step 3a), then rebuild and smoke-test the SW bundle specifically. |
| **8 — DONE** | `frontend/views/**` `.js`: `views/utils` **11**, `containers/chatroom` 5, `chat-mentions` 2, **`voice-recording` 1**, `components/*` 3, `containers/payments` 1, `roles-and-permissions` 1 | **24** | `views/utils` holds 14 entries; one is the `vueComponentStub.js.flow` stub, superseded on the TypeScript side in Step 2 but deleted only in Step 9, and `misc.js` + `allowedUrls.js` went in Step 7, leaving 11. `setupChelonia.js` also went in Step 7 — see "What Step 7 turned up". `voice-recording/voice-recording-utils.js` is new in v2.9.0. View-layer helpers imported by SFCs; SFCs themselves stay untouched plain JS. |
...
### What Step 7 turned up

Twenty-three files converted — the planned 20 plus 3 pulled forward from Step 8 — all green: `tsc` 0 · `eslint` 0 · `npm run flow` "No errors!" · `NODE_ENV=production grunt build` 0 · **178 passing** · 47 → 24 left. 79 specifiers across 60 importers, including `Gruntfile.js:663`. All six contract bundles and `manifests.json` byte-identical raw; `contracts/**` and `chelonia.json` untouched.

**The leaf-first assumption in `tsconfig.json` breaks here, and it is not optional.** That file's `include` comment reasons that a converted file's own imports are already converted, so `.js` never enters the program. Step 7 is the first wave that imports *upward*: `app/group` → `@view-utils/misc` and `@view-utils/allowedUrls`, and `push`/`sw-primary` → `setupChelonia`. A `.ts` file importing a Flow `.js` file makes TypeScript parse it, and Flow syntax there is a **syntax** error — 40 of them, which `checkJs: false` cannot suppress. Two escapes were tested and both fail: an ambient `declare module '@view-utils/misc.js'` does not outrank real-file resolution, and a `tsconfig` `exclude` entry only filters roots (as Step 1 already found). So those three files convert in this wave. Their own import closure adds no further Flow `.js`, and `views/utils/misc.js` alone accounts for 40 of the specifier rewrites. **Before starting Step 8, this direction should be clean — every remaining importer is already `.ts` — but check it the same way: convert, run `tsc`, and read the TS8010/TS1005 list.**

**Service-worker globals: `WorkerGlobalScope` goes global, `self` cannot — and the difference is lib.dom.** Step 6 left this open, and the two names split. Nothing declares `WorkerGlobalScope` (it lives in `lib.webworker`, which tsconfig does not load), so one `declare const WorkerGlobalScope: any` in `declarations.d.ts` serves all three call sites. `self` is the opposite: `lib.dom` already declares it as `Window & typeof globalThis`, and that wins — a global `declare const self: any` is **silently ignored rather than rejected**, and all 43 `self`-dependent errors come back. Verified by removing the per-file declarations with a global one in place. So only module scope shadows `lib.dom`, and `serviceworkers/push.ts`, `serviceworkers/sw-primary.ts` and `model/notifications/nativeNotification.ts` declare it themselves. That is also the narrower change: a global `self` would strip `Window` typing from browser-context code, which is a coverage change rather than parity. Together these cleared **44 errors with zero `@ts-expect-error`**. Untyping `self` also untypes every `self.addEventListener` handler argument, which is what absorbs the `event.waitUntil` / `event.notification` / `event.data` / `event.request` family that lib.dom types as bare `Event`. A controlled Flow probe confirms this is parity and not suppression: `const a: string = self`, `self.totallyMadeUpProperty`, `typeof WorkerGlobalScope` and an unknown property on a handler argument all pass, while the control line errors.

**`@chelonia/crypto`'s `Key` was checked by Flow; `@chelonia/lib`'s `SPKey` was not.** These look alike and are not. `frontend/declarations.js:83` declares `@chelonia/crypto` with a real (if loose) `type Key = { type, secretKey?, publicKey? }` alongside `declare module.exports: any` — so Flow checked the *type* but saw every *value* as `any`. Probe: `const k: Key = 'str'` errors, `const s: SPKey = 12345` does not. Under TypeScript both packages' real declarations apply, so `keygenOfSameType`/`keyId`/`serializeKey` acquire real signatures and an inferred `(string | Key)[]` starts failing as an index type. Bindings are annotated `any` where the real declarations added checking Flow never did — Step 6's `templates.ts` rule — and error-driven, not swept: `SPKey` annotations that cause no error are left as written.

**Restoring parity must not cost a named type — fix the value, not the annotation.** Two sites in this wave were first "fixed" by replacing the type name with `any`, and both were wrong. `e2e/keys.ts` has `{ type = '' }: { type: SPKeyType }`; `SPKeyType` is a union of three key-type literals, so the blocker is the `''` **default**, not the annotation — `{ type = '' as SPKeyType }` keeps the declared type and the runtime value both. `actions/chatroom.ts` annotates `[any, Key, string]` but never imported `Key` (a deleted `$FlowFixMe` had hidden it); its siblings `actions/group.ts` and `actions/identity.ts` import it and typecheck, so the fix is the missing `import type`, not `[any, any, string]`. Deleting a type name to silence an error is a readability regression a reviewer will reject, and `any` is only correct where Flow genuinely checked nothing *and* no narrower spelling compiles.

**`sbp(...args)` needs a tuple.** `@sbp/sbp` ships `declare function sbp(selector: string, ...data: unknown[]): any`, where Flow had `declare module.exports: any`. Spreading a plain array into a signature with a fixed first parameter is TS2556, so the two `const invocation = [...]` literals are annotated `[string, ...any[]]` and the two `sbp(...deserializer(…))` sites are cast. This will recur anywhere an invocation array is built and spread.

**Flow permits calling a declared function type with fewer arguments.** `params.hooks?.onprocessed?.()` against `onprocessed: (Object) => Promise<void>` is fine in Flow and TS2554 in TypeScript. Same family as the unannotated-parameter case in Step 5; same remedy — the parameter becomes optional in `actions/types.ts`.

**The per-file emit check caught the one thing a typecheck cannot, and it is real.** 22 of 23 files are code-identical once deleted `$FlowFixMe` comments are normalised (esbuild keeps comments that sit inside expressions). The exception: esbuild's TS loader **elides `import scrypt from 'scrypt-async'`** from `e2e/keys.ts` because the binding is unused, where the JS loader kept it — the exact risk Step 6 named. It is harmless *only* because that file is dead: nothing imports `frontend/controller/e2e/*`, and `gi.e2e/keys` appears nowhere in `dist/`. **Step 8 must run this check per file rather than assume, and inspect any dropped import for side effects.**

**Two harness traps, both of which report success while measuring nothing.** `flow-remove-types`' CLI writes an empty file, exit 0, when the input extension is unrecognised — so a scratch copy must end in `.js`. And esbuild derives the default-export variable name from the input *basename*, so a comparison keyed on basenames silently collides (`actions/chatroom` vs `app/chatroom`, plus `group`, `identity`, `index`, `keys`, `utils`, `misc`) and reports phantom differences. Use the Node API and key scratch directories on the full path. Step 6's "22 of 24 identical" was produced by a harness without these faults, but it is worth stating them before Step 8 reuses it.

**`sw-primary` output filename is unchanged, as Step 3a predicted.** esbuild still writes `dist/assets/js/sw-primary.js` from a `.ts` entry, so the registration URL at `controller/service-worker.js:119` needs no edit. `service-worker.js` itself stays `.js` — Flow-ignored, and part of Step 9's strip-only set.
...
### What Step 8 turned up

Twenty-four files, all green: `tsc` 0 · `eslint` 0 · `npm run flow` "No errors!" · `NODE_ENV=production grunt build` 0 · **178 passing** · 24 → **0** Flow-checked `.js` left. 79 specifiers across 63 importers, 54 of them `.vue`. All six contract bundles and `manifests.json` byte-identical raw; `contracts/**` and `chelonia.json` untouched. No entry point, `.flowconfig` line or `package.json` field names a file in this wave, so no build config changed — the first wave since Step 3a where that is true.

**Steps 4-8 are complete: 100 Flow-checked files converted** (12 + 17 + 24 + 23 + 24), which reconciles with the scope table. What still contains Flow syntax is the strip-only set for Step 9 — `controller/service-worker.js` and `shared/distribution/distribution.test.js`, both `.flowconfig` `[ignore]` — plus `declarations.js` and the two `.js.flow` stubs.

**`flow-remove-types` does not erase Flow class property declarations, so the plain TypeScript spelling is the emit-identical one.** `AnimationMixins.ts`'s `Confetti` class declares ten (`index: number;`, `tRefMaster: ?number;` …). The expectation was that these are type-only in both languages and that `declare` would be the faithful mirror. Compiling all three ways shows the opposite: `flow-remove-types` blanks each declaration to a bare class field, which esbuild then emits as a `__publicField(this, "index")` call, and plain `index: number` in `.ts` reproduces that call for call. It is `declare index: number` that would have changed the output, by erasing a field the build has always defined. Mirror the annotation and leave the `declare` alone — and note this is `useDefineForClassFields` territory, which `target: ESNext` turns on, so the same reasoning applies to any class field a later wave touches.

**The per-file emit check came out 24 of 24 code-identical**, the single textual difference being one deleted `$FlowFixMe[prop-missing]` comment that esbuild had kept because it sits inside an expression — the same normalisation Steps 6 and 7 needed. Nothing was elided: the `import`-dropping risk Step 7 caught in `e2e/keys.ts` does not recur here.

**`marked` was `declare module.exports: any` to Flow** (`declarations.js:77`), so its real TypeScript declarations check what Flow never did: `marked.parse()` is typed `string | Promise<string>`, and the two `.replace()` calls on the result are TS2339. The binding is annotated `any` — Step 6's `templates.ts` rule, parity rather than suppression. This is the last of these; every remaining `declare module` in `declarations.js` goes away with Flow in Step 9.

**The two remaining TypeScript-only patterns were both already in the table.** `Object.entries(x)` on an `any` yields `unknown` values, so `reduce((str, [from, to]) => …)` needs `[string, any]` (`chat-mentions-utils.ts`, Step 5's idiom); and `let query = { name: content }` seals the type at the initializer, so a later `query = { query: … }` is TS2353, fixed with `: any` (`stepAssistant.ts`, Step 4's `let options = {}`).

**Check the flagged file, not the pattern, before calling something a Flow/TypeScript gap.** `confettiNames`' literal-union annotation looked like plain Flow unsoundness. It is — but confirming it meant putting `const control: string = 42` in that exact file and running `flow check`, which reported the control error and nothing else. That distinguishes "Flow checked this and accepted it" from "Flow never looked", and only the first is a RULES 3 exemption; the second would have been a parity restoration. The check costs one Flow run.
...
- **`model/logServer.ts:5`** — the `Function` return was simply wrong there: every path returns `undefined` and both callers discard it. `any` stands, as already recorded under Deferred.
- **`model/contracts/misc/flowTyper.ts:445`** — `@ts-nocheck` and in `eslintIgnore`, so the alias would be pure decoration, and it is contract source. Not worth the bytes.
- **4 `.vue` sites** (`ProfileCardContent`, `BannerGeneral`, `LoginForm`, `SignupForm`) — SFCs belong to the Vue 3 migration.
...
## Step 9a — Close the coverage gap: the 25 unannotated `.js` files — **DONE**
...
Plus 5 relative-path specifiers. Most importers are `.vue` files. (`@common/common.js` was estimated at 179 across 140 files — the largest in the codebase. The measured figure, once it was converted after all, is **131 across 124**.)
...
3. ~~Then flip `checkJs: true` and add `frontend/**/*.js` to `include`~~ — **done and then reverted; see below.** The renames are what restored the coverage. Fix the errors regardless: ambient `window.Cypress` / `window.sbp` in `declarations.d.ts`, and a decision on `$v`.
4. `common.js` stays `.js` — but not because it is blocked; that was an unverified assumption. See below.

**Gate:** `tsc` 0 (with `checkJs` left off — see below) · 178 passing · `grunt dev` hot-reloads · **contract bundles identical apart from two renamed path comments** — the original wording said "byte-identical", and that was wrong; see below.

---

## What Step 9a turned up

**`common.js` was never blocked — the plan asserted it was without anyone reading the resolver.** See the section above. The conversion was built, verified green, and then reverted by decision; `common.js` is byte-identical to its pre-Step-9a state.

**`checkJs` stayed off, against the plan's own instruction — and with `common.js` staying `.js`, that leaves exactly one file uncovered.**

**The measurement behind that.** Turning it on was step 3 of the order above and a Step 11 acceptance criterion, and it was done — then reverted once the value was measured rather than assumed. By that point the 24 renames had already restored the coverage, because a `.ts` file is checked whatever `checkJs` says. The flip's entire remaining effect was **one file**: `common/common.js`, 15 lines of `export *` re-exports, already clean. Measured both ways — a deliberate `Math.round('a')` in that file is 1 error with it on, 0 with it off, and nothing else in the tree moves. Against that, it costs a standing policy: a developer writing plain JavaScript would find their new `.js` file typechecked by default, and `utils/blockies.js` — vendored, minified, imported by `avatar.ts` past its `exclude` entry — needed a `@ts-nocheck` pragma purely to stay out of the way. Reverted: `checkJs: false`, `include` back to `.ts` only, `blockies.js` back to untouched. **`common.js` is therefore the one file Flow checked under `all=true` that TypeScript does not**, and it is recorded as such rather than papered over.

**Contract CIDs move, and the gate above was written expecting them not to.** `dist/contracts/*.js` is not minified, so esbuild's per-module path banners survive into the shipped bundle, and two of them are renames from this step: `// frontend/utils/events.js` → `.ts` and `// frontend/model/contracts/shared/constants.js` → `.ts`. Measured against a `NODE_ENV=production` build of the Step 9 commit in a worktree: **all six bundles are identical once those two lines are normalised, and nothing else differs at all** — but the lines are real bytes, so all three contract hashes change. This is the class Steps 4-8 already established rather than a new one: 16 of the 17 contract path banners already read `.ts` before this step, and `constants.ts` is simply the last. `contracts/**` and `chelonia.json` stay clean; only a future `grunt pin` makes new CIDs durable. **Build the baseline in a worktree, not from `dist/`** — a stale `dist/` from `grunt test:unit` carries the test-only `forceDistributionDate` / `malformedMutation` actions and will look like a huge diff for an unrelated reason.

**The error count was 10, not 16.** The plan's prose said 16 across 2 files while its own table said 5 and 5; the table was right. All ten are as described, and the `main.js` five break down differently than "one Vue `store` overload" suggested.

**The `store` overload is a module-resolution bug, not a typing gap.** vuex 3.6.0 predates the `types` export condition, so its `exports` map offers `require` and `import` only — `moduleResolution: "bundler"` reads it, finds no `types`, and gives up. `vuex` was therefore an *untyped* module for the whole migration, which is why `types/vue.d.ts` never got a chance to augment `store` onto `ComponentOptions`. Fixed with one `paths` entry pointing at `types/index.d.ts`. It resolves zero new errors' worth of surface, and esbuild ignores it — verified by bundling `import { mapGetters } from 'vuex'` inside the project, which still pulls in the 35 KB runtime rather than the declaration file.

**`$v` is per-component, so a global `Vue` augmentation would have been a false claim.** Vuelidate is never installed with `Vue.use` here; every consumer imports `validationMixin` individually. Declaring `$v` on `vue/types/vue` would assert it exists on every component. `vError.ts` casts `vnode.context` locally instead, which matches what the file already does at runtime — the line below the cast throws `v-error: vuelidate doesn't have validation for …` when it is absent.

**`reducedMotionQuery` errored because Flow's `{}` is unsealed.** `window.matchMedia(…) || {}` reads `.matches` off a `MediaQueryList | {}`; in Flow an empty object literal accepts arbitrary property reads and yields `any`, so it was silent. Annotated `{ matches?: boolean }`, which both sides satisfy and which is the only member the code touches.

**`exclude` does not hold `blockies.js` if `checkJs` is ever turned on.** `@view-utils/avatar.ts` imports it, and an imported file is in the program no matter what `exclude` says — the same trap `service-worker.ts` hit in Step 9. Worth knowing before anyone flips that setting: it needs a `@ts-nocheck` pragma too, the arrangement `flowTyper.ts` and `service-worker.ts` already use.

**`main.ts` needed one Gruntfile edit, not three.** `Gruntfile.js:77` (`mainSrc`) is the entry and had to change. `Gruntfile.js:191` is Browsersync watching the *output* path, and `frontend/index.html:43` loads it — esbuild still emits `dist/assets/js/main.js` from a `.ts` entry, so both were left alone, checked rather than assumed.

**224 specifiers across 164 files**, rewritten by resolving each quoted `.js` specifier through the alias table rather than by pattern — the Step 5 method, reused verbatim. Seven further references were prose, not specifiers, and were updated by hand: four comments naming `constants.js` / `mainNotificationsMixin.js`, a `console.warn` in `avatar.ts` that names its own file, an SCSS comment, and `docs/src/Style-Guide.md`.
...
- [x] `npm run typecheck` clean; `exclude` list matches `.flowconfig` `[ignore]` entry-for-entry (only the 6 stale + 2 redundant omitted, justified in the PR description) — **audited in Step 11**: 14 entries map 1:1; the 8 omitted (`Gruntfile.dashboard.js`, `backend/dashboard/`, `shared/multiformats/`, `shared/blake2bstream.js`, `frontend/utils/vuexQueue.js`, `ignored/`, plus `test/backend.js` and `test/frontend.js`, both already covered by `test/`) all point at paths that no longer exist.
...
- [x] `Gruntfile.js` excluded from `tsc` yet updated as build config — both true, neither an oversight. Confirmed: present in `tsconfig` `exclude`, and +15/−28 across the PR.
...
- [x] `grunt dev` starts, builds, hot-reloads · `NODE_ENV=production grunt build` works — both exit 0. `dev` serves Browsersync 3000 / backend 8000 / dashboard 8888; touching a `.ts` gives `eslint: linted … in 1.8s` then an esbuild rebuild, and a planted unused variable was reported as `@typescript-eslint/no-unused-vars` before being reverted. **That is the only exercise the ported `createEslinter` gets** — `grunt build` lints through the CLI instead.
- [x] `grunt test:unit` passes · `grunt test:cypress` has no new failures vs. the Step 0 baseline — unit **178 passing, 0 failing**; Cypress **163 tests, 153 passing, 0 failing, 10 pending, 13 specs** in 26m27s. Every figure matches 006 exactly, and the 10 pending split the same way (`group-settings` 6, `group-chat` 2, `group-paying` 1, `notifications` 1).
- [x] `npm run lint` and `npm run stylelint` pass · CI green with typecheck in place of Flow — **the script is `npm run eslint`**; both clean, and `--report-unused-disable-directives` reports nothing. **Typecheck reaches CI without a workflow change**: `exec:typecheck` is in `build`'s lint phase (`Gruntfile.js:458`) and `ci-test:unit` calls plain `build`. `.github/workflows/ci.yml` is untouched by this PR and never invoked Flow directly. CI itself still unrun — it needs a push.
- [x] `contracts/` snapshots unchanged; `manifests.json` byte-identical — `contracts/` is byte-identical across the whole PR (`git diff` base..HEAD empty). `manifests.json` is gitignored, so it was checked by rebuilding: the fresh bundles differ from the pinned 2.9.0 snapshots **only in comments** — esbuild's source-path banners following the `.js`→`.ts` renames, the removed `$FlowFixMe`/`$FlowIgnore` markers, and one dead `eslint-disable`. No executable byte moved; esbuild stays 0.27.3 and no bundled dependency changed. Re-pinning remains deferred.
- [x] `flowTyper` equivalence tests pass — all 79 present and green inside the 178.
- [x] **Unit-test suite count matches the Step 0 baseline** — guards against a `*.test.js` rename dropping a suite out of Mocha's glob without failing anything. 178 = 178, with the two renamed specs (`backend.test.ts`, `distribution.test.ts`) visibly present.
...
5a. **Step 9a after Step 9, and its `checkJs` flip last within itself.** The 25 renames are inert; flipping `checkJs` is what changes the gate, so it lands after the renames are proven green — not interleaved with them. Held in practice, and the separation is what made the flip cheap to measure and then drop: renames first (10 errors, all in the two predicted files), flip second (0 further errors), revert third.

...
;(async function () {
  ...
  if (savedStateIndex) {
    // Now, we create the contract state by reading each contract state
    // partition
    // We partition the state into independent slices or partitions per contract
    // for performance reasons (we wouldn't want to serialize and deserialize
    // a single object with potentially thousands or millions of contracts)

...
export default class FsBackend {
  ...
  async testCaseSensitivity () {
    ...
    await this.writeData(originalKey, dateString)
    try {
      const valueOriginalCase = await this.readData(originalKey)
      const valueDifferentCase = await this.readData(differentlyCasedKey)
      ...
      await this.deleteData(originalKey)
    }
  }

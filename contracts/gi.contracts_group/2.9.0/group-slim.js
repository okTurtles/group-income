...
|}`;
  ...
  var group_default = {
    ...
    groupPledgerProfiles(state, getters) {
      return Object.fromEntries(Object.entries(getters.groupProfiles).filter(
        ([memberID, profile]) => profile.incomeDetailsType === "pledgeAmount" && profile.pledgeAmount > 0
      ));
    },
    groupReceiverProfiles(state, getters) {
      return Object.fromEntries(Object.entries(getters.groupProfiles).filter(
        ([memberID, profile]) => profile.incomeDetailsType === "incomeAmount"
      ));
    },
  ...
  var leaveChatRoomAction = async (groupID, state, chatRoomID, memberID, actorID, leavingGroup) => {
    ...
    const groupCSKid = leavingGroup ? await (0, import_sbp3.default)("chelonia/contract/currentKeyIdByName", state, "csk", true) : void 0;
    ...
    (0, import_sbp3.default)("gi.actions/chatroom/leave", {
      ...
      innerSigningContractID: leavingGroup ? null : void 0
    }).catch((e) => {
  ...
  (0, import_sbp3.default)("chelonia/defineContract", {
    ...
    actions: {
      ...
      "gi.contracts/group/removeMember": {
        ...
        process({ data, meta, contractID, height, innerSigningContractID }, { state, getters }) {
          ...
          if (memberID !== identityContractID) {
            (0, import_sbp3.default)("chelonia/contract/setPendingKeyRevocation", state, ["cek", "csk"]);
          }
        },
        ...
        sideEffect({ contractID, data }, { state }) {
          if (data.chatRoomID === state.generalChatRoomId) {
            (0, import_sbp3.default)("chelonia/queueInvocation", contractID, () => {
              ...
              if (state.profiles?.[identityContractID]?.status === PROFILE_STATUS.ACTIVE && state.chatRooms?.[data.chatRoomID]?.members[identityContractID]?.status !== PROFILE_STATUS.ACTIVE) {
                (0, import_sbp3.default)("gi.actions/group/joinChatRoom", {
                  contractID,
                  data: {
            ...
            if (emptyPeriodsBetweenCurrentAndTo.length === 0) {
              ...
              if (groupHasReceivers && allPledgerIds.length > 0) {
                const missedInCurrentPeriod = (memberID) => {
                  return !currentPeriodPayments || !currentPeriodPayments.lastAdjustedDistribution || currentPeriodPayments.lastAdjustedDistribution.some((entry) => entry.fromMemberID === memberID);
                };
                for (const pledgerId of allPledgerIds) {
                  const currentValue = fetchInitKV(state.streaks.missedPayments, pledgerId, 0);
                  state.streaks.missedPayments[pledgerId] = missedInCurrentPeriod(pledgerId) ? 1 + currentValue + emptyPeriodsBetweenCurrentAndTo.length : emptyPeriodsBetweenCurrentAndTo.length;
                }
              }
            }
            state.settings.distributionDate = periodTo;
    ...
    methods: {
      ...
      "gi.contracts/group/_postOpHook/ku": ({ contractID, state }) => {
        (0, import_sbp3.default)("chelonia/queueInvocation", contractID, () => {
          return (0, import_sbp3.default)("gi.actions/group/findAndRequestMissingGroupKeys", contractID, state);
        }).catch((e) => {
      ...
      "gi.contracts/group/leaveGroup": async ({ data, meta, contractID, height, getters, innerSigningContractID, proposalHash }) => {
        ...
        if (memberID === identityContractID) {
          ...
          if (isActionNewerThanUserJoinedDate(height, myProfile)) {
            ...
            const potentialPEKids = state._vm.sharedKeyIds?.filter(({ contractID: contractID2, height: sharedHeight }) => {
              return sharedHeight < height && contractID2 === identityContractID;
            }).map(({ id }) => id);
            Promise.resolve().then(() => (0, import_sbp3.default)("gi.contracts/group/rotateKeys", contractID)).then(() => (0, import_sbp3.default)("gi.contracts/group/revokeGroupKeyAndRotateOurPEK", contractID, potentialPEKids)).catch((e) => {
      ...
      "gi.contracts/group/revokeGroupKeyAndRotateOurPEK": (groupContractID, potentialPEKids) => {
        ...
        (0, import_sbp3.default)("chelonia/queueInvocation", identityContractID, async () => {
          await (0, import_sbp3.default)("chelonia/contract/setPendingKeyRevocation", identityContractID, ["pek"], potentialPEKids);

...
|}`;
  ...
  function updateGroupStreaks({ state, getters }) {
    ...
    for (const username in getters.groupProfiles) {
      ...
      import_common3.Vue.set(streaks.onTimePayments, username, filterMyItems(thisPeriodDistribution, username).length === 0 && filterMyItems(thisPeriodPayments, username).every((p) => p.isLate === false) ? myCurrentStreak + 1 : 0);
    }
  }

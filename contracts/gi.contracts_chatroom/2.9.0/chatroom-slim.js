...
|}`;
  ...
  var ChelErrorInvalidMessageHeight = ChelErrorGenerator("ChelErrorInvalidMessageHeight");
  ...
  var ChelErrorJournalCorrupt = ChelErrorGenerator("ChelErrorJournalCorrupt");
  var ChelErrorKvSlotUnknown = ChelErrorGenerator("ChelErrorKvSlotUnknown");
  var ChelErrorKvSlotInvalid = ChelErrorGenerator("ChelErrorKvSlotInvalid");
  var ChelErrorKvUpdateInvalid = ChelErrorGenerator("ChelErrorKvUpdateInvalid");
  var ChelErrorKvValidation = ChelErrorGenerator("ChelErrorKvValidation");
  var ChelErrorKvConflict = ChelErrorGenerator("ChelErrorKvConflict");
  var ChelErrorKvReentrant = ChelErrorGenerator("ChelErrorKvReentrant");
  ...
  var CHATROOM_MAX_DELETED_MESSAGE_HASHES = 100;
  ...
  async function postLeaveChatRoomCleanup(contractID, state) {
    if (await (0, import_sbp.default)("chelonia/contract/isSyncing", contractID, { firstSync: true })) {
      return;
    }
    (0, import_sbp.default)("gi.actions/identity/kv/deleteChatRoomUnreadMessages", { contractID }).catch((e) => {
      console.error("[leaveChatroom] Error at deleteChatRoomUnreadMessages ", contractID, e);
    });
    (0, import_sbp.default)("okTurtles.events/emit", NEW_CHATROOM_SCROLL_POSITION, { chatRoomID: contractID });
  }
  ...
  var validateChatRoomName = (name) => {
    const nameValidationMap = {
      [(0, import_common2.L)("Chatroom name cannot contain white-space")]: (v) => /\s/.test(v),
      [(0, import_common2.L)("Chatroom name cannot contain capital letters")]: (v) => /\p{Lu}/u.test(v),
      [(0, import_common2.L)("Chatroom name cannot contain punctuation or special characters except hyphens")]: (v) => /[^\p{L}\p{M}\p{Nd}-]/u.test(v)
    };
  ...
  async function deleteEncryptedFiles(manifestCids, option) {
    if (Object.values(option).reduce((a, c) => a || c, false)) {
      ...
      await (0, import_sbp2.default)("gi.actions/identity/removeFiles", { manifestCids, option });
    }
  }
    ...
    actions: {
      ...
      "gi.contracts/chatroom/join": {
        ...
        process({ data, meta, hash, height, contractID, innerSigningContractID }, { state, getters }) {
          ...
          state.members[memberID] = { joinedDate: meta.createdDate, joinedHeight: height, acceptedHeight: isSelf ? height : void 0 };
        ...
        sideEffect({ data, contractID, hash, meta, innerSigningContractID, height }, { state, getters }) {
          const memberID = data.memberID || innerSigningContractID;
          (0, import_sbp2.default)("gi.contracts/chatroom/referenceTally", contractID, memberID, "retain");
          (0, import_sbp2.default)("chelonia/queueInvocation", contractID, async () => {
            ...
            if (!getters.isJoinedChatRoomForChatRoom(state2, memberID)) {
              return;
            }
            const identityContractID = (0, import_sbp2.default)("state/vuex/state").loggedIn.identityContractID;
            if (memberID === identityContractID) {
              await (0, import_sbp2.default)("gi.actions/identity/kv/initChatRoomUnreadMessages", {
                contractID,
                messageHash: hash,
                createdHeight: height
              });
              if (innerSigningContractID !== identityContractID) {
                (0, import_sbp2.default)("gi.actions/chatroom/accept", { contractID, data: null }).catch((e) => {
      ...
      "gi.contracts/chatroom/leave": {
        ...
        process({ data, meta, hash, height, contractID, innerSigningContractID }, { state, getters }) {
          ...
          const itsMe = memberID === (0, import_sbp2.default)("state/vuex/state").loggedIn?.identityContractID;
          if (!itsMe && state.attributes.privacyLevel === CHATROOM_PRIVACY_LEVEL.PRIVATE) {
            (0, import_sbp2.default)("chelonia/contract/setPendingKeyRevocation", state, ["cek", "csk"]);
          }
        },
        async sideEffect({ data, hash, contractID, meta, innerSigningContractID }, { state, getters }) {
          ...
          (0, import_sbp2.default)("gi.contracts/chatroom/referenceTally", contractID, memberID, "release");
          (0, import_sbp2.default)("chelonia/queueInvocation", contractID, async () => {
            const state2 = await (0, import_sbp2.default)("chelonia/contract/state", contractID);
            if (!state2 || getters.isJoinedChatRoomForChatRoom(state2, memberID) || !state2.attributes) {
              return;
            }
            if (!itsMe && state2.attributes.privacyLevel === CHATROOM_PRIVACY_LEVEL.PRIVATE) {
              (0, import_sbp2.default)("gi.contracts/chatroom/rotateKeys", contractID);
            }
            await (0, import_sbp2.default)("gi.contracts/chatroom/removeForeignKeys", contractID, memberID, state2, getters);
          }).catch((e) => {
      ...
      "gi.contracts/chatroom/delete": {
        ...
        async sideEffect({ contractID }, { state }) {
          ...
          const me = (0, import_sbp2.default)("state/vuex/state").loggedIn.identityContractID;
          if (me === state.attributes.creatorID || state.attributes.adminIDs.includes(me)) {
            await (0, import_sbp2.default)("chelonia/out/deleteContract", contractID, {
              [contractID]: {
      ...
      "gi.contracts/chatroom/addMessage": {
        validate: (data, props) => {
          ...
          if (data.type === MESSAGE_TYPES.POLL) {
            ...
            if (data.pollData.question.length > POLL_QUESTION_MAX_CHARS) {
              throw new TypeError((0, import_common3.L)("Poll question must be less than {n} characters", { n: POLL_QUESTION_MAX_CHARS }));
            }
            if (optionStrings.some((v) => v.length > POLL_OPTION_MAX_CHARS)) {
              throw new TypeError((0, import_common3.L)("Poll option must be less than {n} characters", { n: POLL_OPTION_MAX_CHARS }));
            }
          }
        },
        ...
        sideEffect({ contractID, hash, height, meta, data, innerSigningContractID }, { state, getters }) {
          ...
          (0, import_sbp2.default)("chelonia/queueInvocation", contractID, async () => {
            const state2 = await (0, import_sbp2.default)("chelonia/contract/state", contractID);
            if (!getters.isJoinedChatRoomForChatRoom(state2, me)) {
              return;
            }
            const newMessage = createMessage({ meta, data, hash, height, state: state2, innerSigningContractID });
            (0, import_sbp2.default)("okTurtles.events/emit", MESSAGE_RECEIVE_RAW, {
              contractID,
              data,
      ...
      "gi.contracts/chatroom/deleteMessage": {
        ...
        sideEffect({ data, contractID, innerSigningContractID }, { getters }) {
          ...
          if (rootState.chatroom?.chatRoomScrollPosition?.[contractID] === data.hash) {
            (0, import_sbp2.default)("okTurtles.events/emit", NEW_CHATROOM_SCROLL_POSITION, {
          ...
          (0, import_sbp2.default)("chelonia/queueInvocation", contractID, async () => {
            ...
            (0, import_sbp2.default)("gi.actions/identity/kv/removeChatRoomUnreadMessage", { contractID, messageHash: data.hash }).catch((e) => {
    ...
    methods: {
      ...
      "gi.contracts/chatroom/_postOpHook/ku": ({ contractID, state }) => {
        (0, import_sbp2.default)("chelonia/queueInvocation", contractID, () => {
          return (0, import_sbp2.default)("gi.actions/chatroom/findAndRequestMissingChatroomKeys", contractID);
        }).catch((e) => {
      ...
      "gi.contracts/chatroom/_responseOptionsForKeyRequest": ({
        ...
        if (request === "missing" && state.members[originatingContractID] && !state.members[originatingContractID].hasLeft) {
          return {
            keyIds: Object.entries(state._vm.authorizedKeys).filter(([, key]) => !!key.meta?.private?.shareable).map(([kId]) => kId),
      ...
      "gi.contracts/chatroom/_cleanup": ({ contractID, state }) => {
        if (state?.members) {
          ...
          (0, import_sbp2.default)("chelonia/contract/release", activeMemberIds).catch((e) => {
      ...
      "gi.contracts/chatroom/rotateKeys": (contractID) => {
        (0, import_sbp2.default)("chelonia/queueInvocation", contractID, async () => {
          await (0, import_sbp2.default)("gi.actions/out/rotateKeys", contractID, "gi.contracts/chatroom", "pending", "gi.actions/chatroom/shareNewKeys");
        }).catch((e) => {
      ...
      "gi.contracts/chatroom/removeForeignKeys": async (contractID, memberID, state, getters) => {
        const keyIds = await (0, import_sbp2.default)("chelonia/contract/foreignKeysByContractID", state, memberID);
        ...
        (0, import_sbp2.default)("chelonia/out/keyDel", {
          contractID,
          ...
          hooks: {
            preSendCheck: (_, state2) => {
              return !getters.isJoinedChatRoomForChatRoom(state2, memberID);
            }
          }
        }).catch((e) => {

...
export default ({
  ...
  data () {
    return {
      ephemeral: {
        ...
        listener: ({ contractID }) => {
          console.error('@@@@ HOME.VUE ACCEPTED GROUP (cid, currentGID)', contractID, this.currentGroupId)
          ...
          console.error('@@@@ HOME.VUE ACCEPTED GROUP forced outProfileActive to false', contractID, this.currentGroupId)
        }
      }
    }
  },
  beforeMount () {
    ...
    console.error('@@@@ HOME.VUE beforeMount', ACCEPTED_GROUP, this.ephemeral.listener)
  },
  mounted () {
    console.error('@@@@ HOME.VUE MOUNTED', sbp('state/vuex/getters').usernameFromID(sbp('state/vuex/state').loggedIn.identityContractID), this.currentGroupId, this.ourProfileActive, this.$route.query.next)
  ...
  methods: {
    ...
    navigateToGroupPage () {
      ...
      console.error('@@@@ HOME.VUE navigateToGroupPage', path, 'li', this.isLoggedIn, 'ac', this.ephemeral.ourProfileActive, 'n', this.$route.query.next)
      this.$router.push({ path }).catch(e => ignoreWhenNavigationCancelled(e, path))
    }
  },
  watch: {
    currentGroupId (to) {
      console.error('@@@@ HOME.VUE WATCH CURRENTGROUPID', to, this.ephemeral.ourProfileActive, this.ourProfileActive)
      this.ephemeral.ourProfileActive = this.ourProfileActive

...
export function renderMarkdown (str: string): any {
  ...
  // and we don't want this behaviour in the chatroom, so we need to write some custom logic that overrides this.
  const splitByCode = splitStringByMarkdownCode(str)

  splitByCode.forEach((entry: MarkdownSegment, index) => {
    if (entry.type === 'plain' &&
      ...
      splitByCode[index - 1]?.text !== '```'
    ) {
      // special treatment for multiple line-breaks
      // e.g.) \n\n\n -> \n<br><br>
      entry.text = entry.text.replace(/\n{2,}/g, (matchStr) => {
        const numChar = matchStr.length
        const brs = new Array(numChar - 1).fill('<br>').join('')
        return `\n${brs}`
      }).replace(/(|-)(\s.+)\n<br>/g, '-$1\n\n')
    }
  })
  str = combineMarkdownSegmentListIntoString(splitByCode)

  // STEP 2. convert the markdown into html DOM string.
  // -
  let converted = marked.parse(str, { gfm: true, breaks: true })

  // STEP 3. Remove the garbage texts generated by marked.parse()
  // -

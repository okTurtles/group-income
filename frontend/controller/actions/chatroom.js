...
export default (sbp('sbp/selectors/register', {
  ...
  'gi.actions/chatroom/loadChatRoomLogs': async () => {
  ...
  'gi.actions/chatroom/saveChatRoomLogs': (contractID: string, newLogs: Object) => {
    const rootGetters = sbp('state/vuex/getters')
    const { ourIdentityContractId } = rootGetters
  ...
  'gi.actions/chatroom/setChatRoomReadUntil': async ({ contractID, messageHash, createdHeight }: {
    contractID: string, messageHash: string, createdHeight: number
  }) => {
    const currentChatRoomLogs = await sbp('gi.actions/chatroom/loadChatRoomLogs')
    ...
    return sbp('gi.actions/chatroom/saveChatRoomLogs', contractID, currentChatRoomLogs)
  },
  'gi.actions/chatroom/deleteChatRoomReadUntil': async ({ contractID, deletedHeight }: {
    ...
    if (currentChatRoomLogs[contractID]?.readUntil) {
      ...
      await sbp('gi.actions/chatroom/saveChatRoomLogs', contractID, currentChatRoomLogs)
    }
  },
  'gi.actions/chatroom/addChatRoomUnreadMessage': async ({ contractID, messageHash, type, createdHeight }: {
  ...
  'gi.actions/chatroom/deleteChatRoomUnreadMessage': async ({ contractID, messageHash }: {
  ...
  'gi.actions/chatroom/deleteChatRoomLog': async ({ contractID }: { contractID: string }) => {
    const currentChatRoomLogs = await sbp('gi.actions/chatroom/loadChatRoomLogs')

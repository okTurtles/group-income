    ...
    try {
      ...
      // This means that we've been removed from the group
        ...
        sbp('chelonia/queueInvocation', params.contractID, () => {
          ...
          if (state.profiles?.[userID]?.status === PROFILE_STATUS.REMOVED) {
            ...
            console.info('[gi.actions/group/join] We appear to have left the group. Removing the contract.', {
              ...
              hasSecretKeys,
              pendingKeyShares,
              status: state.profiles?.[userID]?.status
            })
            ...
            console.warn('[gi.actions/group/join] Invalid or inconsistent state. We would appear to have been removed but aren\'t', {
              contractID: params.contractID,
              userID,
              hasSecretKeys,
              pendingKeyShares,

...
describe('Test group-income-distribution.js', function () {
  ...
  it('EVENTS: [u1, u2, u3 and u4] join the group and set haveNeeds of [100, 100, -50, and -50], respectively. Test unadjusted.', function () {
    setup.splice(setup.length, 0,
      { type: 'haveNeedEvent', data: { memberID: 'u1', haveNeed: 100 } },
      { type: 'haveNeedEvent', data: { memberID: 'u2', haveNeed: 100 } },
      { type: 'haveNeedEvent', data: { memberID: 'u3', haveNeed: -50 } },
      { type: 'haveNeedEvent', data: { memberID: 'u4', haveNeed: -50 } }
    )
    should(distributionWrapper(setup)).eql([
      { amount: 50, fromMemberID: 'u2', toMemberID: 'u4' },
      { amount: 50, fromMemberID: 'u1', toMemberID: 'u3' }
    ])
  })
  it('Test the adjusted version of the previous event-list. Should be unchanged.', function () {
    should(distributionWrapper(setup, { adjusted: true })).eql([
      { amount: 50, fromMemberID: 'u2', toMemberID: 'u4', total: 50, partial: false, isLate: false, dueOn: '2021-01' },
      { amount: 50, fromMemberID: 'u1', toMemberID: 'u3', total: 50, partial: false, isLate: false, dueOn: '2021-01' }
    ])
  })
  it('EVENT: a payment of $10 is made from u1 to u3.', function () {
    setup.push({ type: 'paymentEvent', data: { fromMemberID: 'u1', toMemberID: 'u3', amount: 10 } })
    should(distributionWrapper(setup, { adjusted: true })).eql([
      { amount: 50, fromMemberID: 'u2', toMemberID: 'u4', total: 50, partial: false, isLate: false, dueOn: '2021-01' },
      { amount: 40, fromMemberID: 'u1', toMemberID: 'u3', total: 50, partial: true, isLate: false, dueOn: '2021-01' }
    ])
  })
  it('EVENT: a payment of $40 is made from u1 to u3.', function () {
    setup.push({ type: 'paymentEvent', data: { fromMemberID: 'u1', toMemberID: 'u3', amount: 40 } })
    should(distributionWrapper(setup, { adjusted: true })).eql([
      { amount: 50, fromMemberID: 'u2', toMemberID: 'u4', total: 50, partial: false, isLate: false, dueOn: '2021-01' }
    ])
  })
